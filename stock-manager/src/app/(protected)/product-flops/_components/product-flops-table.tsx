...
import { createProductsColumns } from "./product-flops-columns";
...
const ProductsTable = ({
  ...
  const [data, setData] = React.useState<ProductFlopRow[]>(initialData);
  ...
  const handleIntervalChange = async (preset: IntervalPreset) => {
    ...
    try {
      const response = await fetch(
        `/api/produtos-encalhados?days=${nextInterval}`,
        {
          cache: "no-store",
        }
      );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Itens com saldo e sem movimentação nos últimos {intervalDays} dias.
          </p>
        </div>
